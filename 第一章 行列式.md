...
[例] 证明：若 $\exists s \neq t, \text{s.t. } \mathbf{a}_s = \mathbf{a}_t$，则 $\det(\mathbf{A}) = 0$。
...
$$
\begin{align}
\hat{\det}(\mathbf{A}_{n \times n}) &= \hat{\det} \left(
\begin{bmatrix} \displaystyle\sum_{j_1 = 1}^n a_{1, j_1} \mathbf{e}_{j_1} \\ \mathbf{a}_2 \\ \vdots \\ \mathbf{a}_n \end{bmatrix} \right)\\
&= \sum_{j_1 = 1}^n a_{1, j_1} \hat{\det} \left( \begin{bmatrix} \mathbf{e}_{j_1} \\ \mathbf{a}_2 \\ \vdots \\ \mathbf{a}_n \end{bmatrix} \right) \\
&= \cdots \\
&= \sum_{j_1 = 1}^n a_{1, j_1} \cdots \sum_{j_n = 1}^n a_{n, j_n} \hat{\det} \left( \begin{bmatrix} \mathbf{e}_{j_1} \\ \mathbf{e}_{j_2} \\ \vdots \\ \mathbf{e}_{j_n} \end{bmatrix} \right) \\
\end{align}
$$

- 由**交错性推论**可知：若 $\exists k_1 \neq k_2, \text{s.t. } j_{k_1} = j_{k_2}$，则 $\hat{\det} \left( \begin{bmatrix} \mathbf{e}_{j_1} \\ \mathbf{e}_{j_2} \\ \vdots \\ \mathbf{e}_{j_n} \end{bmatrix} \right) = 0$，故我们要求 $j$ 是一个 $1 \sim n$ 的排列。
- 对于一个排列 $j$，考虑一个将 $(1, 2, \cdots, n)$ 变换为 $j$ 的长为 $m$ 的对换序列，则由**交错性和单位性**可知 $\hat{\det} \left( \begin{bmatrix} \mathbf{e}_{j_1} \\ \mathbf{e}_{j_2} \\ \vdots \\ \mathbf{e}_{j_n} \end{bmatrix} \right) = (-1)^m \hat{\det} \left( \begin{bmatrix} \mathbf{e}_1 \\ \mathbf{e}_2 \\ \vdots \\ \mathbf{e}_n \end{bmatrix} \right) = (-1)^{\tau(j)} \hat{\det}(\mathbf{I}_n) = (-1)^{\tau(j)}$。
...
- _方便起见，$(1, \cdots, n)$ 中删去 $i$ 可记作 $(1, \cdots, \hat{i}, \cdots, n)$。_

2. 要是我们将 $a_{i, j} A_{i, j}$ 换成 $a_{i, j} A_{i', j}$，其中 $i' \neq i$？

- _Motivation：这一项中就丢失第 $i'$ 行的信息了！_
...
综上，可得 $\displaystyle\sum_{j = 1}^n a_{i, j} A_{i', j} = \delta_{i, i'} |\mathbf{A}|$，其中 $\delta_{i, i'} = [i = i']$ 为克罗克内尔符号 Kronecker symbol。
