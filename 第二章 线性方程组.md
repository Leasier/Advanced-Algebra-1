...
设 $r$ 为变换后阶梯阵中系数矩阵中存在非零项的最后一行。对于 $i \in \mathbb{N}_+, i \leq r$，设 $s_i$ 表示阶梯阵第 $i$ 行首个非零的列指标。
...
记向量 $\mathbf{a} = (a_1, \cdots, a_n)$，其中 $a_i \in \mathbb{F}$。
...
- 注意：负元的存在性是**被定义**的，但这里的 $-\mathbf{a}$ 并非是一种对 $\mathbf{a}$ 的运算，因为我还没有证明其唯一性。
...
- $\mathbf{0}$ 为唯一零元。（证明：假设 $\mathbf{0}^*$ 也是，则用两次单位元性质可知 $\mathbf{0} = \mathbf{0}^*$）

- $\mathbf{0}^*$ 为单位元 $\Leftrightarrow \exists \mathbf{a}^* \in \mathbb{F}^n, \text{s.t. } \mathbf{a}^* + \mathbf{0}^* = \mathbf{a}^*$。（**尽可能不免使用数域自身的性质，而是用抽象出来的向量性质处理。**具体地，我们在证明中将 $\mathbf{0}^*$ 消去。）
...
证明：假设 $\mathbf{a} \in \mathbb{F}^n$ 有不相等的两个逆元 $(-\mathbf{a}), \mathbf{b}$，则：
$$
...
- $\lambda \cdot \mathbf{a} = \mathbf{0} \Leftrightarrow \lambda = 0 \lor \mathbf{a} = \mathbf{0}$。
...
定义向量 $\mathbf{e}_i$ 满足第 $j$ 个分量为 $[j = i]$，则我们可以将任意向量 $\mathbf{a}$ **唯一地**表示为 $\displaystyle\sum_{i = 1}^n a_i \mathbf{e}_i$。
...
设 $\mathbf{a}_i = (a_{i, 1}, \cdots, a_{i, n})$，考察线性方程组 $\forall j \in \mathbb{N}, j \leq n, \displaystyle\sum_{i = 1}^m a_{i, j} x_i = 0$。
