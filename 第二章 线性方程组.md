...
设 $\mathbf{a}_i = (a_{1, i}, \cdots, a_{n, i}) \in \mathbb{F}^n$，考察线性方程组 $\forall j \in \mathbb{N}_+ \land j \leq n, \displaystyle\sum_{i = 1}^m a_{j, i} x_i = 0$。
...
#### 定理：$\{\mathbf{a}_1, \cdots, \mathbf{a}_m\}$ 线性相关当且进度 $\exists \mathbf{a}_i, \text{s.t. }$ 其可由其他向量线性表出。
...
- 进而有 $\displaystyle\sum_{j = 1}^m \lambda_j \mathbf{a}_j = 0$，其中 $\lambda_i = -1$。
...
- 取 $\mu_1, \cdots, \mu_s \in \mathbb{F}$ 且这 $s$ 个数不全为 $0$，则 $\displaystyle\sum_{i = 1}^s \mu_i \mathbf{b}_i = \sum_{j = 1}^m (\sum_{i = 1}^s \lambda_{i, j} \mu_i) \mathbf{a}_j$。
- 要使 $\{\mathbf{b}_1, \cdots, \mathbf{b}_s\}$ 线性相关，只要 $\displaystyle\sum_{i = 1}^s \mu_i = 0$，进而只要 $\forall j \in \mathbb{N}_+ \land j \leq m, \displaystyle\sum_{i = 1}^s \lambda_{i, j} \mu_i = 0$。
...
向 $\{\mathbf{a}_1, \cdots, \mathbf{a}_m\}$ 中添加 $s - m$ 个向量变成 $\{\mathbf{a}'_1, \cdots, \mathbf{a}'_s\}$，后者称作前者的扩充，前者称作后者的缩减。
...
将向量 $\mathbf{a} = (a_1, \cdots, a_n) \in \mathbb{F}^n$ 变换为向量 $\mathbf{a}' = (a'_1, \cdots, a'_s) \in \mathbb{F}^s$，其中 $s > n$，使得 $\forall i \in \mathbb{N}_+ \land i \leq n, a'_i = a_i$，后者称作前者的接长，前者称作后者的截短。

##### 定理：接长线性无关的向量组得到的一定是线性无关的向量组，截短线性相关的向量组得到的一定是线性相关的向量组。

- **Motivation**：方程数增多后，新方程组的解集是原方程组的解集的子集。
...
从 $\{\mathbf{a}_1, \cdots, \mathbf{a}_m\}$ 中选出 $\{\mathbf{a}_{i_1}, \cdots, \mathbf{a}_{i_r}\}$，使得：
...
##### 命题：$\forall k \in \mathbb{N}_+ \land k \leq m, a_k$ 可由 $\{\mathbf{a}_{i_1}, \cdots, \mathbf{a}_{i_r}\}$ 线性表出。

证明：

- 由于 $\{\mathbf{a}_{i_1}, \cdots, \mathbf{a}_{i_r}, \mathbf{a}_k\}$ 线性相关，$\exists \lambda_1, \cdots, \lambda_r, \mu \in \mathbb{F}$ 且这 $r + 1$ 个数非全 $0$，使得 $\displaystyle\sum_{j = 1}^r \lambda_j \mathbf{a}_{i_j} + \mu \mathbf{a}_k = 0$。
...
$\{\mathbf{a}_1, \cdots, \mathbf{a}_m\}$ 的极大无关组中的向量个数，称为其秩，记作 $\operatorname{rank}(\mathbf{a}_1, \cdots, \mathbf{a}_m)$。

##### 求法

法一：逐个检验是否能被先前加入者线性表出。

法二：将 $\mathbf{a}_i$ 作为**行向量**堆叠成矩阵 $A_{m \times n}$，施高斯消去法得阶梯阵，取非零部分，考察它是如何被表出的。
