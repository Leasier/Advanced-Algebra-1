...
可以发现，上面的证明用到了 $V$ 为**有限维**线性空间的性质。事实上，当 $V$ 为**无限维**线性空间，有如下反例：
...
可以发现这个变换阵呈现出某种对称性。事实上，令 $\mathbf{S} = \begin{bmatrix} a_0 & -b_0 \\ b_0 & a_0 \end{bmatrix}, \mathbf{T} = \begin{bmatrix} c_0 & -d_0 \\ d_0 & c_0 \end{bmatrix}$，则变换阵也可以写成较简洁的分块矩阵的形式：
...
[例 7] 求证：$\mathbb{F}$ 上的 $n$ 阶方阵 $\mathbf{A}$ 满足 $\mathbf{A}^2 - 2 \mathbf{A} + \mathbf{I}_n = \mathbf{O}$ 当且仅当 $\exists s \in \mathbb{N}_{\leq n}, \text{s.t. } \mathbf{A} \sim \begin{bmatrix} \mathbf{I}_s & \mathbf{C}_{s \times (n - s)} \\ \mathbf{O} & \mathbf{I}_{n - s} \end{bmatrix}$。

令 $\mathbf{B} = \mathbf{A} - \mathbf{I}$，则命题等价于：$\mathbf{B}^2 = \mathbf{O}$ 当且仅当 $\exists s \in \mathbb{N}_{\leq n}, \text{s.t. } \mathbf{B} \sim \begin{bmatrix} \mathbf{O} & \mathbf{C}_{s \times (n - s)} \\ \mathbf{O} & \mathbf{O} \end{bmatrix}$。

充分性显然，下证必要性。

法一：利用相似与线性变换的关系（**构造性证明**）

- 只需证明存在一组基，使得左乘 $\mathbf{B}$ 的变换作用于这组基上的变换阵形如 $\begin{bmatrix} \mathbf{O} & \mathbf{C} \\ \mathbf{O} & \mathbf{O} \end{bmatrix}$。
- _Motivation：首先需要选择一组基，使得 $\mathbf{B}$ 乘上其中每个基向量都得到 $\mathbf{0}$。_
- 令 $r = \operatorname{rank}(\mathbf{B})$，取 $\mathcal{R}(\mathbf{B})$ 的一组基 $\{\mathbf{B} \mathbf{e}_1, \cdots, \mathbf{B} \mathbf{e}_r\}$，并将其补全为 $\mathbb{F}^n$ 上的一组基 $\{\mathbf{B} \mathbf{e}_1, \cdots, \mathbf{B} \mathbf{e}_r, \mathbf{f}_1, \cdots, \mathbf{f}_{n - r}\}$。
- 则 $\exists \mathbf{C} \in \mathbb{F}^{r \times (n - r)}, \text{s.t. } \mathbf{B} \begin{bmatrix} \mathbf{f}_1, \cdots, \mathbf{f}_{n - r} \end{bmatrix} = \begin{bmatrix} \mathbf{B} \mathbf{e}_1, \cdots, \mathbf{B} \mathbf{e}_r \end{bmatrix} \mathbf{C}$。
- 于是 $\mathbf{B} \begin{bmatrix} \mathbf{B} \mathbf{e}_1, \cdots, \mathbf{B} \mathbf{e}_r, \mathbf{f}_1, \cdots, \mathbf{f}_{n - r} \end{bmatrix} = \begin{bmatrix} \mathbf{B} \mathbf{e}_1, \cdots, \mathbf{B} \mathbf{e}_r, \mathbf{f}_1, \cdots, \mathbf{f}_{n - r} \end{bmatrix} \begin{bmatrix} \mathbf{O} & \mathbf{C} \\ \mathbf{O} & \mathbf{O} \end{bmatrix}$。
- 故 $\mathbf{B} \sim \begin{bmatrix} \mathbf{O} & \mathbf{C} \\ \mathbf{O} & \mathbf{O} \end{bmatrix}$，遂得证。

法二：相抵标准形（**分块矩阵的代数处理，存在性证明**）

- 令 $r = \operatorname{rank}(\mathbf{B})$，则 $\exists \mathbf{P}, \mathbf{Q}$ 非奇异，$\text{s.t. } \mathbf{B} = \mathbf{P} \begin{bmatrix} \mathbf{I}_r & \mathbf{O} \\ \mathbf{O} & \mathbf{O} \end{bmatrix} \mathbf{Q}$。
- 由 $\mathbf{B}^2 = \mathbf{O}$ 可知 $\mathbf{P} \begin{bmatrix} \mathbf{I}_r & \mathbf{O} \\ \mathbf{O} & \mathbf{O} \end{bmatrix} \mathbf{Q} \mathbf{P} \begin{bmatrix} \mathbf{I}_r & \mathbf{O} \\ \mathbf{O} & \mathbf{O} \end{bmatrix} \mathbf{Q} = \mathbf{O}$。
- 令 $\mathbf{R} = \mathbf{Q} \mathbf{P} = \begin{bmatrix} \mathbf{R}_{1, 1} & \mathbf{R}_{1, 2} \\ \mathbf{R}_{2, 1} & \mathbf{R}_{2, 2} \end{bmatrix}$，则 $\begin{bmatrix} \mathbf{I}_r & \mathbf{O} \\ \mathbf{O} & \mathbf{O} \end{bmatrix} \mathbf{R} \begin{bmatrix} \mathbf{I}_r & \mathbf{O} \\ \mathbf{O} & \mathbf{O} \end{bmatrix} = \mathbf{O}$，可得 $\mathbf{R}_{1, 1} = \mathbf{O}$。
- 故 $\mathbf{B} = \mathbf{P} \begin{bmatrix} \mathbf{I}_r & \mathbf{O} \\ \mathbf{O} & \mathbf{O} \end{bmatrix} (\mathbf{R} \mathbf{P}^{-1}) = \mathbf{P} \begin{bmatrix} \mathbf{O} & \mathbf{R}_{1, 2} \\ \mathbf{O} & \mathbf{O} \end{bmatrix} \mathbf{P}^{-1}$，遂得证。
...
- 由**同构**关系 $V \cong \mathbb{F}^n$ 可知 $V$ 中的线性相关性与其坐标向量的线性相关性等价，故 $\{\tilde{\mathbf{a}}_1, \cdots, \tilde{\mathbf{a}}_n\}$（即 $\mathbf{A}$ 的列向量组）与 $\{\mathcal{A} \mathbf{e}_1, \cdots, \mathcal{A} \mathbf{e}_n\}$ 的极大无关组对应一致，均为 $r$ 个。
...
推论：有限维线性空间上的单射、满射和双射等价。

事实上，上面的证明可以直接从**线性变换**扩展到**线性映射**的情况（即 $\mathcal{A} \in \mathcal{L} (V, W)$）。

[例 8] 对于欧氏空间 $U, V$，在 $U$ 上定义内积 $\forall \boldsymbol{\alpha}, \boldsymbol{\beta} \in U, (\boldsymbol{\alpha}, \boldsymbol{\beta}) \in \mathbb{R}$，在 $V$ 上定义内积 $\forall \mathbf{a}, \mathbf{b} \in V, [\mathbf{a}, \mathbf{b}] \in \mathbb{R}$。

若 $\mathcal{A}$ 为 $U \to V$ 的线性映射、$\mathcal{B}$ 为 $V \to U$ 的线性映射，且 $\forall \boldsymbol{\alpha} \in U, \mathbf{a} \in V, [\mathcal{A} \boldsymbol{\alpha}, \mathbf{a}] = (\boldsymbol{\alpha}, \mathcal{B} \mathbf{a})$：

- (1) 求证：$\dim (\mathcal{A} U) = \dim (\mathcal{B} V)$。
- (2) 设 $Z = \{\mathbf{b} \mid \mathcal{B} \mathbf{b} = \mathbf{0}\} \subset V$，求证：$\forall \mathbf{a} \in V$，$\exists \boldsymbol{\alpha} \in U, \text{s.t. } \mathcal{A} \boldsymbol{\alpha} = \mathbf{a}$ 的充要条件为 $\forall \mathbf{b} \in Z, [\mathbf{a}, \mathbf{b}] = 0$。

证明 (1)：

- _Motivation：能不能取一些较为特殊的 $\boldsymbol{\alpha}, \mathbf{a}$？_
- 注意到当 $\mathcal{A} \boldsymbol{\alpha} = \mathbf{0}_V$，必有 $(\boldsymbol{\alpha}, \mathcal{B} \mathbf{a}) = [\mathbf{0}_V, \mathbf{a}] = 0$。
- 则 $\forall \mathbf{v} \in \ker(\mathcal{A}) \cap \mathcal{R} (\mathcal{B}), (\mathbf{v}, \mathbf{v}) = 0 \Rightarrow \mathbf{v} = \mathbf{0}_U$，即 $\ker(\mathcal{A}) + \mathcal{R} (\mathcal{B})$ 为直和。
- 故 $\dim(\ker(\mathcal{A}) \oplus \mathcal{R} (\mathcal{B})) = \dim \ker(\mathcal{A}) + \dim \mathcal{R} (\mathcal{B}) \leq \dim U$。同理有 $\dim(\ker(\mathcal{B}) \oplus \mathcal{R} (\mathcal{A})) = \dim \ker(\mathcal{B}) + \dim \mathcal{R} (\mathcal{A}) \leq \dim V$。
- _Motivation：$\ker$ 和 $\mathcal{R}$？是不是可以考虑维数定理？_
- 由维数定理可知 $\dim \mathcal{R} (\mathcal{A}) + \dim \ker(\mathcal{A}) = \dim U, \dim \mathcal{R} (\mathcal{B}) + \dim \ker(\mathcal{B}) = \dim V$，相加可知上面两个不等式同时取等。
- 联立 $\dim \ker(\mathcal{A}) + \dim \mathcal{R} (\mathcal{B}) = \dim U$ 和 $\dim \mathcal{R} (\mathcal{A}) + \dim \ker(\mathcal{A}) = \dim U$ 可得 $\dim \mathcal{R} (\mathcal{A}) = \dim \mathcal{R} (\mathcal{B})$，即 $\dim (\mathcal{A} U) = \dim (\mathcal{B} V)$。

证明 (2)：

- 必要性：当 $\exists \boldsymbol{\alpha} \in U, \text{s.t. } \mathcal{A} \boldsymbol{\alpha} = \mathbf{a}$，有 $\forall \mathbf{b} \in Z, [\mathbf{a}, \mathbf{b}] = [\mathcal{A} \boldsymbol{\alpha}, \mathbf{b}] = (\boldsymbol{\alpha}, \mathcal{B} \mathbf{b}) = (\boldsymbol{\alpha}, \mathbf{0}_U) = 0$。
- 充分性：由 (1) 中不等式取等可推知 $\ker(\mathcal{B}) \oplus \mathcal{R} (\mathcal{A}) = V$。
- 由直和可知 $\exists ! \mathbf{a}_1 \in \ker(\mathcal{B}), \mathbf{a}_2 \in \mathcal{R} (\mathcal{A}), \text{s.t. } \mathbf{a} = \mathbf{a}_1 + \mathbf{a}_2$。
- $\forall \mathbf{b} \in Z, 0 = [\mathbf{a}, \mathbf{b}] = [\mathbf{a}_1, \mathbf{b}] + [\mathbf{a}_2, \mathbf{b}]$。
- 由 $\exists \boldsymbol{\alpha} \in U, \text{s.t. } \mathcal{A} \boldsymbol{\alpha} = \mathbf{a}_2$，同“必要性”可知 $[\mathbf{a}_2, \mathbf{b}] = 0$，故 $[\mathbf{a}_1, \mathbf{b}] = 0$。
- 取 $\mathbf{b} = \mathbf{a}_1$ 可知 $[\mathbf{a}_1, \mathbf{a}_1] = 0$，则 $\mathbf{a}_1 = \mathbf{0}_V$。
- 故 $\mathbf{a} = \mathbf{a}_2 \in \mathcal{R} (\mathcal{B})$。
