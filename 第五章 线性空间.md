...
内容：若 $\{\mathbf{e}_1, \cdots, \mathbf{e}_n\}$ 为线性空间 $(V; +, \cdot; \mathbb{F})$ 的一组基，且 $\forall i \in [1, s] \cap \mathbb{N}_+, \mathbf{a}_i \in V$ 在这组基下的坐标为 $\mathbf{x}_i \in \mathbb{F}^n$，则 $\{\mathbf{a}_1, \cdots, \mathbf{a}_s\}$ 线性相关当且仅当 $\{\mathbf{x}_1, \cdots, \mathbf{x}_s\}$ 线性相关。
...
$$
\begin{aligned}
\sum_{i = 1}^s \lambda_i \mathbf{x}_i &= \sum_{i = 1}^s \lambda_i \sum_{j = 1}^n x_{i, j} \mathbf{e}_j \\
&= \sum_{j = 1}^n (\sum_{i = 1}^s \lambda_i x_{i, j}) \mathbf{e}_j \\
&= \sum_{j = 1}^n 0 \cdot \mathbf{e}_j \\
&= \mathbf{0}
\end{aligned}
...
- (2) 令 $\tilde{\mathbf{e}}_i = \dfrac{\overline{\mathbf{e}}_i}{||\overline{\mathbf{e}}_i||}$，则 $\tilde{\mathbf{e}}_i$ 为单位向量，且继承了 $\overline{\mathbf{e}}_i$ 与 $\forall j \in [1, i) \cap \mathbb{N}, \tilde{\mathbf{e}}_j$ 的正交性。
