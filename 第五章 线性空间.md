...
[例 15] 求证：$\forall \mathbb{R}^n$ 的一个子空间 $V$，$\exists m \in \mathbb{N}, \mathbf{A} \in \mathbb{R}^{m \times n}, \text{s.t. } V = \ker(\mathbf{A})$。
...
我们称这样定义的 $\tilde{V}$ 为 $V_1, V_2$ 的外直和。
...
对于 $\mathbb{F}$ 上的线性空间 $V, W$，若存在**一一映射 / 双射** bijective $\sigma : V \to W$，且 $\sigma$ **保持线性性**，则称 $V$ 与 $W$ 同构，记作 $V \cong W$。

- $\sigma$ 保持线性性指 $\forall \lambda, \mu \in \mathbb{F}, \mathbf{a}, \mathbf{b} \in V, \sigma (\lambda \mathbf{a} + \mu \mathbf{b}) = \lambda \sigma (\mathbf{a}) + \mu \sigma (\mathbf{b})$。

#### 定理：线性空间的同构是等价关系。

证明：

- (1) 自反性：构造恒同映射 $\mathcal{I}_d$：

$$
\begin{aligned}
\mathcal{I}_d : \ &V \to V \\
&\mathbf{a} \mapsto \mathbf{b} = \mathbf{a}
\end{aligned}
$$

- I. $\forall \mathbf{a} \in V, \mathcal{I}_d (\mathbf{a}) = \mathbf{a}$，则 $\mathcal{I}_d$ 为满射。
- II. $\forall \mathbf{a}_1, \mathbf{a}_2 \in V \land \mathbf{a}_1 \neq \mathbf{a}_2, \mathcal{I}_d (\mathbf{a}_1) = \mathbf{a}_1 \neq \mathbf{a}_2 = \mathcal{I}_d (\mathbf{a}_2)$，则 $\mathcal{I}_d$ 为单射。
- III. $\forall \lambda, \mu \in \mathbb{F}, \mathbf{a}_1, \mathbf{a}_2 \in V, \mathcal{I}_d (\lambda \mathbf{a}_1 + \mu \mathbf{a}_2) = \lambda \mathbf{a}_1 + \mu \mathbf{a}_2 = \lambda \mathcal{I}_d (\mathbf{a}_1) + \mu \mathcal{I}_d (\mathbf{a}_2)$，则 $\mathcal{I}_d$ 保持线性性。
- 综合 I、II、III 可知 $\mathcal{I}_d : V \to V$ 是保持线性性的一一映射，故 $V$ 与 $V$ 同构。
- (2) 对称性：已知 $V$ 与 $W$ 同构，欲证 $W$ 与 $V$ 同构。
- 设 $\sigma : V \to W$ 是保持线性性的一一映射，则可定义其逆映射 $\sigma^{-1}$：
- I. $\forall \mathbf{b} \in W$，由于 $\sigma$ 为满射，$\exists \mathbf{a} \in V, \text{s.t. } \mathbf{b} = \sigma (\mathbf{a})$；又由于 $\sigma$ 为单射，这样的 $\mathbf{a}$ 唯一；故可定义 $\sigma^{-1}(\mathbf{b}) = \mathbf{a}$。
- II. $\forall \mathbf{a} \in V, \sigma^{-1}(\sigma (\mathbf{a})) = \mathbf{a}$，则 $\sigma^{-1}$ 为满射。
- III. $\forall \mathbf{b}_1, \mathbf{b}_2 \in W \land \mathbf{b}_1 \neq \mathbf{b}_2$，若 $\sigma^{-1}(\mathbf{b}_1) = \sigma^{-1}(\mathbf{b}_2)$ 则 $\mathbf{b}_1 = \sigma (\sigma^{-1}(\mathbf{b}_1)) = \sigma (\sigma^{-1}(\mathbf{b}_2)) = \mathbf{b}_2$，与 $\mathbf{b}_1 \neq \mathbf{b}_2$ 矛盾，故 $\sigma^{-1}(\mathbf{b}_1) \neq \sigma^{-1}(\mathbf{b}_2)$，则 $\sigma^{-1}$ 为单射。
- IV. $\forall \lambda, \mu \in \mathbb{F}, \mathbf{b}_1, \mathbf{b}_2 \in W, \sigma^{-1}(\lambda \mathbf{b}_1 + \mu \lambda{b}_2) = \sigma^{-1}(\lambda \sigma (\sigma^{-1}(\mathbf{b}_1)) + \mu \sigma (\sigma^{-1}(\mathbf{b}_2))) = \sigma^{-1}(\sigma (\lambda \sigma^{-1}(\mathbf{b}_1) + \mu \sigma^{-1}(\mathbf{b}_2))) = \lambda \sigma^{-1}(\mathbf{b}_1) + \mu \sigma^{-1}(\mathbf{b}_2)$，则 $\sigma^{-1}$ 保持线性性。
- 综合 I、II、III、IV 可知 $\sigma^{-1} : W \to V$ 是保持线性性的一一映射，故 $W$ 与 $V$ 同构。
- (3) 传递性：已知 $V$ 与 $W$ 同构、$W$ 与 $U$ 同构，欲证 $V$ 与 $W$ 同构。
- 设 $\sigma : V \to W, \tau : W \to U$ 是保持线性性的一一映射，由 (2) 可知两者的逆 $\sigma^{-1} : W \to V, \tau^{-1} : U \to W$ 也是保持线性性的一一映射。
- 进而可给出其复合 $\tau \circ \sigma$：

$$
\begin{aligned}
\tau \circ \sigma : \ &V \to U \\
&\mathbf{a} \mapsto \mathbf{b} = \tau(\sigma (\mathbf{a}))
\end{aligned}
$$

- I. $\forall \mathbf{b} \in U, (\tau \circ \sigma)(\sigma^{-1}(\tau^{-1}(\mathbf{b}))) = \mathbf{b}$，则 $\tau \circ \sigma$ 为满射。
- II. $\forall \mathbf{a}_1, \mathbf{a}_2 \in V \land \mathbf{a}_1 \neq \mathbf{a}_2, \sigma (\mathbf{a}_1) \neq \sigma (\mathbf{a}_2), (\tau \circ \sigma)(\mathbf{a}_1) = \tau (\sigma (\mathbf{a}_1)) \neq \tau (\sigma (\mathbf{a}_2)) = (\tau \circ \sigma)(\mathbf{a}_2)$，则 $\tau \circ \sigma$ 为单射。
- III. $\forall \lambda, \mu \in \mathbb{F}, \mathbf{a}_1, \mathbf{a}_2 \in V, (\tau \circ \sigma)(\lambda \mathbf{a}_1 + \mu \mathbf{a}_2) = \tau (\sigma (\lambda \mathbf{a}_1 + \mu \mathbf{a}_2)) = \tau (\lambda \sigma (\mathbf{a}_1) + \mu \sigma (\mathbf{a}_2)) = \lambda \tau (\sigma (\mathbf{a}_1)) + \mu \tau (\sigma (\mathbf{a}_2)) = \lambda (\tau \circ \sigma)(\mathbf{a}_1) + \mu (\tau \circ \sigma)(\mathbf{a}_2)$。
- 综合 I、II、III 可知 $\tau \circ \sigma : V \to U$ 是保持线性性的一一映射，故 $V$ 与 $U$ 同构。

#### 定理（有限维线性空间的约化）

内容：对于 $\mathbb{F}$ 上的 $n$ 维线性空间 $V$，有 $V \cong \mathbb{F}^n$。

证明：

- 取 $V$ 的一组基 $\{\mathbf{e}_1, \cdots, \mathbf{e}_n\}$，构造映射 $\sigma$：

$$
\begin{aligned}
\sigma : \ &V \to \mathbb{F} \\
&\mathbf{v} \mapsto \mathbf{x} = (x_1, \cdots, x_n), \text{if } \mathbf{v} = \sum_{i = 1}^n x_i \mathbf{e}_i
\end{aligned}
$$
...
- 同一数域上相同有限维数的线性空间同构。
- $\mathbb{R}$ 上的行向量线性空间和 $\mathbb{R}$ 上的列向量线性空间同构。

#### 定理（同构保持向量组的线性相关性）

内容：若 $V, W$ 是 $\mathbb{F}$ 上的线性空间，$\sigma : V \to W$ 为同构映射，且 $\forall i \in [1, n] \cap \mathbb{N}, \sigma (\mathbf{a}_i) = \mathbf{b}_i$，则 $\{\mathbf{a}_1, \cdots, \mathbf{a}_n\}$ 线性相关 $\Leftrightarrow \{\mathbf{b}_1, \cdots, \mathbf{b}_n\}$ 线性相关。

证明：

- $\Rightarrow$：若 $\{\mathbf{a}_1, \cdots, \mathbf{a}_n\}$ 线性相关，则 $\exists \lambda_1, \cdots, \lambda_n \in \mathbb{F}$ 非全零，$\text{s.t. } \displaystyle\sum_{i = 1}^n \lambda_i \mathbf{a}_i = \mathbf{0}$。
- 由 $\sigma$ 保持线性性可知 $\mathbf{0} = \sigma(\mathbf{0}) = \displaystyle\sum_{i = 1}^n \lambda_i \sigma (\mathbf{a}_i) = \sum_{i = 1}^n \lambda_i \mathbf{b}_i$，则 $\{\mathbf{b}_1, \cdots, \mathbf{b}_n\}$ 线性相关。
- $\Leftarrow$：利用 $\sigma^{-1} : W \to V$ 为同构映射，同理可证。

推论：$\{\mathbf{a}_1, \cdots, \mathbf{a}_n\}$ 线性无关 $\Leftrightarrow \{\mathbf{b}_1, \cdots, \mathbf{b}_n\}$ 线性无关。
