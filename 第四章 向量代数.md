...
- 此时称 $L(A; \mathbf{a})$ **在** $\Pi (O; \mathbf{b}, \mathbf{c})$ **外**，且 $L(A; \mathbf{a})$ 与 $\Pi (O; \mathbf{b}, \mathbf{c})$ **平行**（即平面内存在该直线的平行线）。
...
证明：$\overrightarrow{AD} = \dfrac{1}{2} (\overrightarrow{AB} + \overrightarrow{AC}) \Leftrightarrow \overrightarrow{AD} = \dfrac{1}{2} ((\overrightarrow{AD} - \overrightarrow{BD}) + (\overrightarrow{AD} - \overrightarrow{CD})) \Leftrightarrow \overrightarrow{BD} + \overrightarrow{CD} = \mathbf{0} \Leftrightarrow D$ 为 $BC$ 中点。
...
- 注意表述中体现的“继承”思想：二维**不共线** $\Rightarrow$ 三维**不共面**。
...
- **注意：这里仅仅是记号，而非表示行向量与列向量的乘法，因为 $\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3$ 是几何向量而非代数向量、没法被塞进矩阵里。**

对于三维空间中任意点 $A$，$\overrightarrow{OA}$ 称为 $A$ 的矢径 / 向径，称 $\overrightarrow{OA}$ 的坐标为 $A$ 的坐标。

我们将平面坐标系中“四象限”的概念拓展到仿射坐标系中“八卦限”的概念：对于坐标 $a_1, a_2, a_3 \neq 0, (a_1, a_2, a_3) \in \mathbb{R}^3$，定义其卦限为 $1 + [(a_1 < 0) \neq (a_2 < 0)] + [a_2 < 0] 2 + [a_3 < 0] 4$。
...
\Leftrightarrow &(\mathbf{a}, \mathbf{b}) \leq \sqrt{(\mathbf{a}, \mathbf{a}) (\mathbf{b}, \mathbf{b})} \land -(\mathbf{a}, \mathbf{b}) \leq \sqrt{(\mathbf{a}, \mathbf{a}) (\mathbf{b}, \mathbf{b})} \\
...
特别地，当 $\langle \mathbf{a}, \mathbf{b} \rangle = 0$，有 $(\mathbf{a}, \mathbf{b}) = ||\mathbf{a}|| \cdot ||\mathbf{b}||$，令 $\lambda = -\dfrac{||\mathbf{a}||}{||\mathbf{b}||}$ 即得 $||\mathbf{a} + \lambda \mathbf{b}|| = 0$，有 $\mathbf{a} = -\lambda \mathbf{b} = \dfrac{||\mathbf{a}||}{||\mathbf{b}||} \cdot \mathbf{b}$，故 $\mathbf{a}, \mathbf{b}$ 共线。

#### 定义：向量的垂直 / 正交 Orthogonal

对于内积空间 $V$ 上的 $\mathbf{a}, \mathbf{b}$，定义 $\mathbf{a}, \mathbf{b}$ 垂直当且仅当 $(\mathbf{a}, \mathbf{b}) = 0$。

- 特别地，当 $\mathbf{a}, \mathbf{b}$ 之一为零，上面的定义仍给出 $\mathbf{a}, \mathbf{b}$ 垂直，这样的定义符合我们对“零向量方向任意”的理解。
...
对内积空间 $V$ 上的 $\mathbf{a}, \mathbf{b}$，当 $\mathbf{b} \neq \mathbf{0}$，定义 $\mathbf{a}$ 在 $\mathbf{b}$ 上的投影**向量** $\mathbb{P}_{\mathbf{b}}(\mathbf{a}) = \dfrac{(\mathbf{a}, \mathbf{b})}{||\mathbf{b}||} \cdot \dfrac{\mathbf{b}}{||\mathbf{b}||} = (\mathbf{a}, \dfrac{\mathbf{b}}{||\mathbf{b}||^2}) \mathbf{b}$，又记作 $\mathbf{a}_{\mathbf{b}}^{\parallel}$。

在此基础上，令 $\mathbf{a}_{\mathbf{b}}^{\perp} = \mathbf{a} - \mathbf{a}_{\mathbf{b}}^{\parallel}$。易证 $\mathbf{a}_{\mathbf{b}}^{\parallel} \parallel \mathbf{b}, \mathbf{a}_{\mathbf{b}}^{\perp} \perp \mathbf{b}$。
...
- 必要性：由定义易知。
- 充分性：$\forall L (O'; \mathbf{d}) \subset \Pi (O; \mathbf{b}, \mathbf{c})$，显然有 $\Pi (O; \mathbf{b}, \mathbf{c}) = \Pi (O'; \mathbf{x}, \mathbf{y})$。
...
- 故 $(\mathbf{a}, \mathbf{d}) = \lambda (\mathbf{a}, \mathbf{x}) + \mu (\mathbf{a}, \mathbf{y}) = 0$，于是 $L(A; \mathbf{a}) \perp L(O'; \mathbf{d})$。
...
- 由于 $\begin{vmatrix} (\mathbf{b}, \mathbf{b}) & (\mathbf{c}, \mathbf{b}) \\ (\mathbf{b}, \mathbf{c}) & (\mathbf{c}, \mathbf{c}) \end{vmatrix} = (\mathbf{b}, \mathbf{b}) (\mathbf{c}, \mathbf{c}) - (\mathbf{b}, \mathbf{c})^2 = (\mathbf{b}, \mathbf{b}) (\mathbf{c}, \mathbf{c}) (1 - \cos^2 \langle \mathbf{b}, \mathbf{c} \rangle) > 0$（由于 $\mathbf{b}, \mathbf{c}$ 不共线，无法取等），可得以上关于 $x, y$ 的二元一次方程有唯一解，因此满足条件的 $A'$ 存在。
...
- **“坐标系”的抽象：我们不想去关心具体的“坐标”这样的 "dirty work"，而是更期待利用内积的性质。**
...
而我们只需证明 $AH \perp BC \Leftrightarrow (\mathbf{a}, \mathbf{b} - \mathbf{c}) = 0 \Leftrightarrow (\mathbf{a}, \mathbf{b}) = (\mathbf{a}, \mathbf{c})$。
...
内容：在 $\Delta ABC$ 内部取一点 $O$，连接 $AO, BO, CO$ 并延长、分别交 $BC, AC, AB$ 于 $D, E, F$，则 $\dfrac{|CD|}{|BD|} \cdot \dfrac{|AE|}{|CE|} \cdot \dfrac{|BF|}{|AF|} = 1$。

证明：

- _Motivation：我想保全对称性。_
- 考虑以 $O$ 为原点建立坐标系，设 $A, B, C$ 的矢径分别为 $\mathbf{a}, \mathbf{b}, \mathbf{c}$。
- 设：

$$
\begin{cases}
\lambda \in (0, 1), |CD| : |BD| = \lambda : (1 - \lambda) \\
\mu \in (0, 1), |AE| : |CE| = \mu : (1 - \mu) \\
\theta \in (0, 1), |BF| : |AF| = \theta : (1 - \theta)
\end{cases}
$$
...
$$
\begin{cases}
\overrightarrow{OD} = \lambda \mathbf{b} + (1 - \lambda) \mathbf{c} \\
\overrightarrow{OE} = \mu \mathbf{c} + (1 - \mu) \mathbf{a} \\
\overrightarrow{OF} = \theta \mathbf{a} + (1 - \theta) \mathbf{b}
\end{cases}
$$

- 由于 $(A, O, D), (B, O, E), (C, O, F)$ 分别共线，有：

$$
\begin{cases}
\exists \alpha \neq 0, \text{s.t. } \alpha \mathbf{a} + \overrightarrow{OD} = \mathbf{0} \Leftrightarrow \alpha \mathbf{a} + \lambda \mathbf{b} + (1 - \lambda) \mathbf{c} = \mathbf{0} \\
\exists \beta \neq 0, \text{s.t. } \beta \mathbf{b} + \overrightarrow{OE} = \mathbf{0} \Leftrightarrow (1 - \mu) \mathbf{a} + \beta \mathbf{b} + \mu \mathbf{c} = \mathbf{0} \\
\exists \gamma \neq 0, \text{s.t. } \theta \mathbf{c} + \overrightarrow{OF} = \mathbf{0} \Leftrightarrow \theta \mathbf{a} + (1 - \theta) \mathbf{b} + \gamma \mathbf{c} = \mathbf{0}
\end{cases}
$$

- 由 $\mathbf{a}, \mathbf{b}, \mathbf{c}$ 不共线，知 $\exists ! (x_0, y_0) \in (\mathbb{R}_{\neq 0})^2, \text{s.t. } \mathbf{c} = x_0 \mathbf{a} + y_0 \mathbf{b}$，故所有非零且满足 $x \mathbf{a} + y \mathbf{b} + z \mathbf{c} = \mathbf{0}$ 的 $(x, y, z)$ 都可以表示为 $t(x_0, y_0, -1)$ 的形式，即**系数成比例**。
- 记三式系数分别为 $\mathcal{A}, \mathcal{B}, \mathcal{C}$，则：

$$
\begin{cases}
\dfrac{\mathcal{A}}{\mathcal{B}} = \dfrac{1 - \lambda}{\mu} \\
\dfrac{\mathcal{B}}{\mathcal{C}} = \dfrac{1 - \mu}{\theta} \\
\dfrac{\mathcal{C}}{\mathcal{A}} = \dfrac{1 - \theta}{\lambda}
\end{cases}
$$

- 三式相乘可知 $1 = \dfrac{\mathcal{A}}{\mathcal{B}} \cdot \dfrac{\mathcal{B}}{\mathcal{C}} \cdot \dfrac{\mathcal{C}}{\mathcal{A}} = \dfrac{1 - \lambda}{\mu} \cdot \dfrac{1 - \mu}{\theta} \cdot \dfrac{1 - \theta}{\lambda}$，即 $\dfrac{|CD|}{|BD|} \cdot \dfrac{|AE|}{|CE|} \cdot \dfrac{|BF|}{|AF|} = 1$。
...
设空间中规定顺序的**不共面**向量组 $(\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3)$ 为一组标架。
...
- (3) 传递性：$|\mathbf{T}_{\mathbf{A}, \mathbf{B}}|, |\mathbf{T}_{\mathbf{B}, \mathbf{C}}| > 0 \Rightarrow |\mathbf{T}_{\mathbf{A}, \mathbf{C}}| = |\mathbf{T}_{\mathbf{A}, \mathbf{B}} \mathbf{T}_{\mathbf{B}, \mathbf{C}}| = |\mathbf{T}_{\mathbf{A}, \mathbf{B}}| |\mathbf{T}_{\mathbf{B}, \mathbf{C}}| > 0$。
...
- “右手系”只需表述为“与右手确定的某个标架具有同一定向的标架给出的仿射坐标系”，它无关数学结构本身，因此这个定义自身是“干净”的。

不难发现，给一个标架中任一向量数乘正数后得到的新标架与原标架定向相同，数乘负数后得到的新标架与原标架定向相反。

#### 定义：外积 Exterior / Wedge Product

在三维空间 $V$ 上，选定标架 $\mathbf{E} = (\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3)$，定义外积为一个函数 $V \times V \to V$，记作 $\mathbf{a} \times \mathbf{b}$，满足：

- (1) 初始值：$\mathbf{e}_2 \times \mathbf{e}_3 = \mathbf{e}_1, \mathbf{e}_3 \times \mathbf{e}_1 = \mathbf{e}_2, \mathbf{e}_1 \times \mathbf{e}_2 = \mathbf{e}_3$。
- (2) **反**对称性：$\mathbf{a} \times \mathbf{b} = -(\mathbf{b} \times \mathbf{a})$。
- (3) 线性性：$(\lambda \mathbf{a} + \mu \mathbf{b}) \times \mathbf{c} = \lambda (\mathbf{a} \times \mathbf{c}) + \mu (\mathbf{b} \times \mathbf{c})$。

考察仿射坐标系 $\{O; \mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3\}$ 下，坐标表示的向量的外积：
...
&= (a_1 b_1) (\mathbf{e}_1 \times \mathbf{e}_1) + (a_1 b_2 - a_2 b_1) (\mathbf{e}_1 \times \mathbf{e}_2) + (a_1 b_3 - a_3 b_1) (\mathbf{e}_1 \times \mathbf{e}_3) + (a_2 b_2) (\mathbf{e}_2 \times \mathbf{e}_2) + (a_2 b_3 - a_3 b_2) (\mathbf{e}_2 \times \mathbf{e}_3) + (a_3 b_3) (\mathbf{e}_3 \times \mathbf{e}_3) \\
&= \begin{vmatrix} a_2 & a_3 \\ b_2 & b_3 \end{vmatrix} \mathbf{e}_1 - \begin{vmatrix} a_1 & a_3 \\ b_1 & b_3 \end{vmatrix} \mathbf{e}_2 + \begin{vmatrix} a_1 & a_2 \\ b_1 & b_2 \end{vmatrix} \mathbf{e}_3 \\
&\equiv \begin{vmatrix} \mathbf{e}_1 & \mathbf{e}_2 & \mathbf{e}_3 \\ a_1 & a_2 & a_3 \\ b_1 & b_2 & b_3 \end{vmatrix}
\end{aligned}
$$

- **注意：这里最后一行仅为记号，而非真正的行列式！**~~这样写只是因为它在此确实遵循了真正的行列式的运算规律（~~

显然，当 $\mathbf{a}, \mathbf{b}$ 共线，$\mathbf{a} \times \mathbf{b} = \mathbf{0}$。

#### 外积的性质

在 $V$ 上定义内积，使得 $(\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3)$ 构成**单位正交基**，并由内积诱导出范数。

对于向量 $\mathbf{a} = a_1 \mathbf{e}_1 + a_2 \mathbf{e}_2 + a_3 \mathbf{e}_3, \mathbf{b} = b_1 \mathbf{e}_1 + b_2 \mathbf{e}_2 + b_3 \mathbf{e}_3$，首先来计算 $\mathbf{a} \times \mathbf{b}$ 的长度：

$$
\begin{aligned}
||\mathbf{a} \times \mathbf{b}|| &= \sqrt{\begin{vmatrix} a_2 & a_3 \\ b_2 & b_3 \end{vmatrix}^2 + \begin{vmatrix} a_3 & a_1 \\ b_3 & b_1 \end{vmatrix}^2 + \begin{vmatrix} a_1 & a_2 \\ b_1 & b_2 \end{vmatrix}^2} \\
&= (a_1^2 + a_2^2 + a_3^2)(b_1^2 + b_2^2 + b_3^2) - (a_1 b_1 + a_2 b_2 + a_3 b_3)^2 \\
&= \sqrt{||\mathbf{a}||^2 ||\mathbf{b}||^2 - (\mathbf{a}, \mathbf{b})^2} \\
&= \sqrt{||\mathbf{a}||^2 ||\mathbf{b}||^2 - ||\mathbf{a}||^2 ||\mathbf{b}||^2 \cos \langle \mathbf{a}, \mathbf{b} \rangle^2} \\
&= ||\mathbf{a}|| \cdot ||\mathbf{b}|| \cdot \sin \langle \mathbf{a}, \mathbf{b} \rangle
\end{aligned}
$$

因此，几何上，外积的长度恰为以 $\mathbf{a}, \mathbf{b}$ 为两边的平行四边形的面积。

接下来，对于**不共线**的 $\mathbf{a}, \mathbf{b}$，计算从 $\mathbf{E}$ 到 $(\mathbf{a}, \mathbf{b}, \mathbf{a} \times \mathbf{b})$ 的过渡阵的行列式：

$$
\begin{aligned}
\begin{vmatrix}
a_1 & b_1 & \begin{vmatrix} a_2 & a_3 \\ b_2 & b_3 \end{vmatrix} \\
a_2 & b_2 & -\begin{vmatrix} a_1 & a_3 \\ b_1 & b_3 \end{vmatrix} \\
a_3 & b_3 & \begin{vmatrix} a_1 & a_2 \\ b_1 & b_2 \end{vmatrix}
\end{vmatrix}
&= \begin{vmatrix} a_2 & a_3 \\ b_2 & b_3 \end{vmatrix} \cdot \begin{vmatrix} a_2 & a_3 \\ b_2 & b_3 \end{vmatrix} - (-\begin{vmatrix} a_1 & a_3 \\ b_1 & b_3 \end{vmatrix}) \cdot \begin{vmatrix} a_1 & a_3 \\ b_1 & b_3 \end{vmatrix} + \begin{vmatrix} a_1 & a_2 \\ b_1 & b_2 \end{vmatrix} \cdot \begin{vmatrix} a_1 & a_2 \\ b_1 & b_2 \end{vmatrix} \\
&= \begin{vmatrix} a_2 & a_3 \\ b_2 & b_3 \end{vmatrix}^2 + \begin{vmatrix} a_1 & a_3 \\ b_1 & b_3 \end{vmatrix}^2 + \begin{vmatrix} a_1 & a_2 \\ b_1 & b_2 \end{vmatrix}^2
\end{aligned}
$$

由于 $\mathbf{a}, \mathbf{b}$ 不共线，$\begin{vmatrix} a_2 & a_3 \\ b_2 & b_3 \end{vmatrix}, \begin{vmatrix} a_1 & a_3 \\ b_1 & b_3 \end{vmatrix}, \begin{vmatrix} a_1 & a_2 \\ b_1 & b_2 \end{vmatrix}$ 中至少一个非零，故行列式大于零，则 $(\mathbf{a}, \mathbf{b}, \mathbf{a} \times \mathbf{b})$ 与 $\mathbf{E}$ 定向一致。

[例 3] 求证：若 $\boldsymbol{\alpha} \times \boldsymbol{\beta} = \boldsymbol{\beta} \times \boldsymbol{\gamma} = \boldsymbol{\gamma} \times \boldsymbol{\alpha} \neq \mathbf{0}$，则 $\boldsymbol{\alpha} + \boldsymbol{\beta} + \boldsymbol{\gamma} = \mathbf{0}$。

- _Motivation：关注目标。_

令 $\mathbf{v} = \boldsymbol{\alpha} + \boldsymbol{\beta} + \boldsymbol{\gamma}$，则：

$$
\begin{aligned}
\boldsymbol{v} \times \boldsymbol{\alpha} &= \boldsymbol{\alpha} \times \boldsymbol{\alpha} + \boldsymbol{\beta} \times \boldsymbol{\alpha} + \boldsymbol{\gamma} \times \boldsymbol{\alpha} \\
&= \mathbf{0} + (-\boldsymbol{\alpha} \times \boldsymbol{\beta}) + \boldsymbol{\gamma} \times \boldsymbol{\alpha} \\
&= \mathbf{0}
\end{aligned}
$$

同理有 $\mathbf{v} \times \boldsymbol{\beta} = \mathbf{0}$。

由 $\boldsymbol{\alpha} \times \boldsymbol{\beta} \neq \mathbf{0}$ 可知 $\boldsymbol{\alpha}, \boldsymbol{\beta} \neq \mathbf{0}$ 且**不共线**，由 $\mathbf{v} \times \boldsymbol{\alpha} = \mathbf{v} \times \boldsymbol{\beta} = \mathbf{0}$ 可知 $\mathbf{v}, \boldsymbol{\alpha}$ 和 $\mathbf{v}, \boldsymbol{\beta}$ 分别**共线**。

若 $\mathbf{v} \neq \mathbf{0}$，则 $\boldsymbol{\alpha}, \boldsymbol{\beta}$ 共线，矛盾。故 $\mathbf{v} = \mathbf{0}$ 即 $\boldsymbol{\alpha} + \boldsymbol{\beta} + \boldsymbol{\gamma} = \mathbf{0}$。

[例 4] 求证：$\boldsymbol{\alpha} \times \boldsymbol{\beta} + \boldsymbol{\beta} \times \boldsymbol{\gamma} + \boldsymbol{\gamma} \times \boldsymbol{\alpha} = \mathbf{0}$ 当且仅当 $\exists c_1, c_2, c_3 \in \mathbb{R}$ 且非全零，$\text{s.t. } c_1 + c_2 + c_3 = 0, c_1 \boldsymbol{\alpha} + c_2 \boldsymbol{\beta} + c_3 \boldsymbol{\gamma} = \mathbf{0}$。

必要性：

- 由 $c_1 + c_2 + c_3 = 0, c_1 \boldsymbol{\alpha} + c_2 \boldsymbol{\beta} + c_3 \boldsymbol{\gamma} = \mathbf{0}$ 可知 $c_1 (\boldsymbol{\alpha} - \boldsymbol{\gamma}) + c_2 (\boldsymbol{\beta} - \boldsymbol{\gamma}) = \mathbf{0}$。
- 由 $c_1, c_2, c_3$ 非全零可知 $c_1, c_2$ 非全零，故 $\boldsymbol{\alpha} - \boldsymbol{\gamma}, \boldsymbol{\beta} - \boldsymbol{\gamma}$ 共线，则 $(\boldsymbol{\alpha} - \boldsymbol{\gamma}) \times (\boldsymbol{\beta} - \boldsymbol{\gamma}) = \mathbf{0}$。
- 展开可知 $\boldsymbol{\alpha} \times \boldsymbol{\beta} - \boldsymbol{\alpha} \times \boldsymbol{\gamma} - \boldsymbol{\gamma} \times \boldsymbol{\beta} + \boldsymbol{\gamma} \times \boldsymbol{\gamma} = \mathbf{0}$，即 $\boldsymbol{\alpha} \times \boldsymbol{\beta} + \boldsymbol{\beta} \times \boldsymbol{\gamma} + \boldsymbol{\gamma} \times \boldsymbol{\alpha} = \mathbf{0}$，遂得证。

充分性反过来写一遍上面的过程即可，略。

#### 定义：混合积 Mixed Product

在三维空间 $V$ 上，对于标架 $\mathbf{S} = (\mathbf{i}, \mathbf{j}, \mathbf{k})$ 导出的外积 $\times$ 和内积 $\cdot$，定义混合积为一个函数 $V \times V \times V \to \mathbb{R}$，记作 $(\mathbf{a}, \mathbf{b}, \mathbf{c})$，满足 $(\mathbf{a}, \mathbf{b}, \mathbf{c}) = (\mathbf{a} \times \mathbf{b}) \cdot \mathbf{c}$。
...
- (3) 线性性：$(\lambda \mathbf{a} + \mu \mathbf{b}, \mathbf{c}, \mathbf{d}) = \lambda (\mathbf{a}, \mathbf{c}, \mathbf{d}) + \mu (\mathbf{b}, \mathbf{c}, \mathbf{d})$，对另外两个参数同样也有线性性。
- (4) 消零性：$(\mathbf{a}, \mathbf{a}, \mathbf{b}) = 0$。

(1)(3)(4) 的证明是显然的。

为证明 (2)，考察直角坐标系（**注意内积是 $\mathbf{S}$ 导出的，这意味着 $\mathbf{i}, \mathbf{j}, \mathbf{k}$ 被“定义”为一组单位正交基**）$\{O; \mathbf{i}, \mathbf{j}, \mathbf{k}\}$ 下，坐标表示的向量的混合积：
...
有趣的是，若从**变换阵**的视角看待 $\begin{vmatrix} x_a & y_a & z_a \\ x_b & y_b & z_b \\ x_c & y_c & z_c \end{vmatrix}$ 对应的矩阵，则可以发现：

$$
\begin{bmatrix} \mathbf{i} & \mathbf{j} & \mathbf{k} \end{bmatrix}
\begin{bmatrix}
x_a & x_b & x_c \\
y_a & y_b & y_c \\
z_a & z_b & z_c
...
即当该矩阵非奇异，从标架 $\mathbf{I}$ 到标架 $(\mathbf{a}, \mathbf{b}, \mathbf{c})$ 的变换阵 $\mathbf{T}$ 恰为其转置。

故混合积 $(\mathbf{a}, \mathbf{b}, \mathbf{c}) = |\mathbf{T}^T| = |\mathbf{T}|$，则这两个标架定向相同当且仅当 $(\mathbf{a}, \mathbf{b}, \mathbf{c}) > 0$。

------

在 (2) 的基础上，考察一般的仿射坐标系 $\{O; \mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3\}$ 下，坐标表示的向量的混合积：
...
$$
\begin{aligned}
(\mathbf{a}, \mathbf{b}, \mathbf{c}) &= (\mathbf{a} \times \mathbf{b}) \times \mathbf{c} \\
&= (\begin{vmatrix} a_2 & a_3 \\ b_2 & b_3 \end{vmatrix} (\mathbf{e}_2 \times \mathbf{e}_3) - \begin{vmatrix} a_1 & a_3 \\ b_1 & b_3 \end{vmatrix} (\mathbf{e}_3 \times \mathbf{e}_1) + \begin{vmatrix} a_1 & a_2 \\ b_1 & b_2 \end{vmatrix} (\mathbf{e}_1 \times \mathbf{e}_2)) \cdot (c_1 \mathbf{e}_1 + c_2 \mathbf{e}_2 + c_3 \mathbf{e}_3) \\
&= c_1 \begin{vmatrix} a_2 & a_3 \\ b_2 & b_3 \end{vmatrix} (\mathbf{e}_2 \times \mathbf{e}_3, \mathbf{e}_1) - c_2 \begin{vmatrix} a_1 & a_3 \\ b_1 & b_3 \end{vmatrix} (\mathbf{e}_3 \times \mathbf{e}_1, \mathbf{e}_2) + c_3 \begin{vmatrix} a_1 & a_2 \\ b_1 & b_2 \end{vmatrix} (\mathbf{e}_1 \times \mathbf{e}_2, \mathbf{e}_3) \\
&= (c_1 \begin{vmatrix} a_2 & a_3 \\ b_2 & b_3 \end{vmatrix} - c_2 \begin{vmatrix} a_1 & a_3 \\ b_1 & b_3 \end{vmatrix} + c_3 \begin{vmatrix} a_1 & a_2 \\ b_1 & b_2 \end{vmatrix}) (\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3) \\
&= \begin{vmatrix} a_1 & a_2 & a_3 \\ b_1 & b_2 & b_3 \\ c_1 & c_2 & c_3 \end{vmatrix} (\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3)
\end{aligned}
$$

[例 5] 在空间直角坐标系中，四面体顶点 $A, B, C, D$ 的坐标分别为 $(1, 0, 1), (-1, 1, 5), (-1, -3, -3), (0, 3, 4)$，求其体积。
...
则四面体体积为这三条边张成的平行六面体体积的 $\dfrac{1}{6}$，用混合积计算即得：

$$
\begin{aligned}
V &= \frac{1}{6} |(\overrightarrow{AB}, \overrightarrow{AB}, \overrightarrow{AD})| \\
&= \frac{1}{6} \left| \det \left( \begin{bmatrix} -2 & 1 & 4 \\ -2 & -3 & -4 \\ -1 & 3 & 3 \end{bmatrix} \right) \right| \\
&= \frac{16}{3}
\end{aligned}
$$
...
现有右手系 $\{O; \mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3\}$ 和该系下的 $\mathbf{a} = a_1 \mathbf{e}_1 + a_2 \mathbf{e}_2 + a_3 \mathbf{e}_3$，我们想要求出其在 $\{O'; \mathbf{f}_1, \mathbf{f}_2, \mathbf{f}_3\}$ 下的坐标 $\mathbf{a} = a'_1 \mathbf{f}_1 + a'_2 \mathbf{f}_2 + a'_3 \mathbf{f}_3$。

若已知从 $(\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3)$ 到 $(\mathbf{f}_1, \mathbf{f}_2, \mathbf{f}_3)$ 的过渡阵为 $\mathbf{T} = \begin{bmatrix} \tilde{\mathbf{t}}_1 & \tilde{\mathbf{t}}_2 & \tilde{\mathbf{t}}_3 \end{bmatrix}$，一个最暴躁的想法是解线性方程组：

$$
\begin{bmatrix} a_1 \\ a_2 \\ a_3 \end{bmatrix} = \mathbf{T} \begin{bmatrix} a'_1 \\ a'_2 \\ a_3' \end{bmatrix} 
$$

方便起见，记**代数向量** $\tilde{\mathbf{a}} = \begin{bmatrix} a_1 \\ a_2 \\ a_3 \end{bmatrix}$，则由 Cramer 法则可知：

$$
\begin{cases}
a'_1 = \dfrac{\begin{vmatrix} \tilde{\mathbf{a}} & \mathbf{t}_2 & \mathbf{t}_3 \end{vmatrix}}{|\mathbf{T}|} \\
a'_2 = \dfrac{\begin{vmatrix} \mathbf{t}_1 & \tilde{\mathbf{a}} & \mathbf{t}_3 \end{vmatrix}}{|\mathbf{T}|} \\
a'_3 = \dfrac{\begin{vmatrix} \mathbf{t}_1 & \mathbf{t}_2 & \tilde{\mathbf{a}} \end{vmatrix}}{|\mathbf{T}|}
\end{cases}
$$

与此同时，我们可以采取另一个视角：

- 注意到 $(\mathbf{a}, \mathbf{f}_2, \mathbf{f}_3) = a'_1 (\mathbf{f}_1, \mathbf{f}_2, \mathbf{f}_3) + a'_2 (\mathbf{f}_2, \mathbf{f}_2, \mathbf{f}_3) + a'_3 (\mathbf{f}_3, \mathbf{f}_2, \mathbf{f}_3) = a'_1 (\mathbf{f}_1, \mathbf{f}_2, \mathbf{f}_3)$。
- 故 $a'_1 = \dfrac{(\mathbf{a}, \mathbf{f}_2, \mathbf{f}_3)}{(\mathbf{f}_1, \mathbf{f}_2, \mathbf{f}_3)}$，利用混合积的行列式形式展开，消去 $(\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3)$ 因子即得。
- $a'_2, a'_3$ 的计算是同理的。

#### 定义：复合积

在三维空间 $V$ 上，对于标架 $\mathbf{S} = (\mathbf{i}, \mathbf{j}, \mathbf{k})$ 导出的外积 $\times$，称 $(\mathbf{a} \times \mathbf{b}) \times \mathbf{c}$ 为复合积。
...
在直角坐标系 $\{O; \mathbf{i}, \mathbf{j}, \mathbf{k}\}$ 下，考虑用**内积**得到在各坐标轴上的投影，即计算 $(\mathbf{a} \times \mathbf{b}) \times \mathbf{c}$ 与各基向量的内积：
...
&= - \begin{vmatrix} x_a & x_a x_c + y_a y_c + z_a z_c \\ x_b & x_b x_c + y_b y_c + z_b z_c \end{vmatrix} \\
...
进而求得 $(\mathbf{a} \times \mathbf{b}) \times \mathbf{c} = (\mathbf{a} \cdot \mathbf{c}) \mathbf{b} - (\mathbf{b} \cdot \mathbf{c}) \mathbf{a}$。同理有 $\mathbf{a} \times (\mathbf{b} \times \mathbf{c}) = (\mathbf{a} \cdot \mathbf{c}) \mathbf{b} - (\mathbf{b} \cdot \mathbf{a}) \mathbf{c}$。

[例 6] 设 $\boldsymbol{\alpha} \neq \mathbf{0}, \boldsymbol{\alpha} \perp \boldsymbol{\beta}$，已知 $\boldsymbol{\xi}$ 满足 $\boldsymbol{\alpha} \cdot \boldsymbol{\xi} = c, \boldsymbol{\alpha} \times \boldsymbol{\xi} = \boldsymbol{\beta}$，求证：$\boldsymbol{\xi} = \dfrac{c \boldsymbol{\alpha} - \boldsymbol{\alpha} \times \boldsymbol{\beta}}{||\boldsymbol{\alpha}||^2}$。

法一：利用复合积

- _Motivation：我想构造 $\boldsymbol{\alpha} \times \boldsymbol{\beta}$。_
- 在 $\boldsymbol{\alpha} \times \boldsymbol{\xi} = \boldsymbol{\beta}$ 两侧同时左乘 $\boldsymbol{\alpha}$，得到 $\boldsymbol{\alpha} \times \boldsymbol{\beta} = \boldsymbol{\alpha} \times (\boldsymbol{\alpha} \times \boldsymbol{\xi}) = (\boldsymbol{\alpha} \cdot \boldsymbol{\xi}) \boldsymbol{\alpha} - (\boldsymbol{\alpha} \cdot \boldsymbol{\alpha}) \boldsymbol{\xi} = c \boldsymbol{\alpha} - ||\boldsymbol{\alpha}||^2 \boldsymbol{\xi}$。
- 移项整理即得 $\boldsymbol{\xi} = \dfrac{c \boldsymbol{\alpha} - \boldsymbol{\alpha} \times \boldsymbol{\beta}}{||\boldsymbol{\alpha}||^2}$。

法二：利用向量共面的性质

- 由 $\boldsymbol{\alpha} \times \boldsymbol{\xi} = \boldsymbol{\beta}$ 可知 $\boldsymbol{\xi} \perp \boldsymbol{\beta}$。
- 故与 $\boldsymbol{\beta}$ 相垂直的两个不共线非零向量即可张出 $\boldsymbol{\xi}$。
- 由 $\boldsymbol{\alpha} \neq \mathbf{0}, \boldsymbol{\alpha} \perp \boldsymbol{\beta}$ 知其一可选 $\boldsymbol{\alpha}$，易得另一个选 $\boldsymbol{\alpha} \times \boldsymbol{\beta}$ 即可。
- 列线性表出的方程即可，略。

#### 定理：拉格朗日 Lagrange 公式

内容：$(\mathbf{a} \times \mathbf{b}) \cdot (\mathbf{c} \times \mathbf{d}) = (\mathbf{a}, \mathbf{c}) (\mathbf{b}, \mathbf{d}) - (\mathbf{a}, \mathbf{d}) (\mathbf{b}, \mathbf{c})$。

证明：

$$
\begin{aligned}
(\mathbf{a} \times \mathbf{b}) \cdot (\mathbf{c} \times \mathbf{d}) &= (\mathbf{c} \times \mathbf{d}) \cdot (\mathbf{a} \times \mathbf{b}) \\
&= (\mathbf{c}, \mathbf{d}, \mathbf{a} \times \mathbf{b}) \\
&= (\mathbf{a} \times \mathbf{b}, \mathbf{c}, \mathbf{d}) \\
&= ((\mathbf{a} \times \mathbf{b}) \times \mathbf{c}) \cdot \mathbf{d} \\
&= ((\mathbf{a} \cdot \mathbf{c}) \mathbf{b} - (\mathbf{b}, \mathbf{c}) \mathbf{a}) \cdot \mathbf{d} \\
&= (\mathbf{a} \cdot \mathbf{c}) (\mathbf{b} \cdot \mathbf{d}) - (\mathbf{a}, \mathbf{d}) (\mathbf{b}, \mathbf{c})
\end{aligned}
$$

答案也可以写成行列式形式 $\begin{bmatrix} (\mathbf{a}, \mathbf{c}) & (\mathbf{b}, \mathbf{c}) \\ (\mathbf{a}, \mathbf{d}) & (\mathbf{b}, \mathbf{d}) \end{bmatrix}$。

[例 7] 求证：对于标架 $\mathbf{S}$ 导出的外积 $\times$，若 $\boldsymbol{\alpha}, \boldsymbol{\beta}, \boldsymbol{\gamma}$ 不共面，则 $\boldsymbol{\beta} \times \boldsymbol{\gamma}, \boldsymbol{\gamma} \times \boldsymbol{\alpha}, \boldsymbol{\alpha} \times \boldsymbol{\beta}$ 也不共面，且定向与 $\mathbf{S}$ 一致。

$$
\begin{aligned}
(\boldsymbol{\beta} \times \boldsymbol{\gamma}, \boldsymbol{\gamma} \times \boldsymbol{\alpha}, \boldsymbol{\alpha} \times \boldsymbol{\beta}) &= ((\boldsymbol{\beta} \times \boldsymbol{\gamma}) \times (\boldsymbol{\gamma} \times \boldsymbol{\alpha})) \cdot (\boldsymbol{\alpha} \times \boldsymbol{\beta}) \\
&= \begin{bmatrix} (\boldsymbol{\beta} \times \boldsymbol{\gamma}, \mathbf{\boldsymbol{\alpha}}) & (\boldsymbol{\beta} \times \boldsymbol{\gamma}, \mathbf{\boldsymbol{\beta}}) \\ (\boldsymbol{\gamma} \times \boldsymbol{\alpha}, \mathbf{\boldsymbol{\alpha}}) & (\boldsymbol{\gamma} \times \boldsymbol{\alpha}, \mathbf{\boldsymbol{\beta}}) \end{bmatrix} \\
&= (\boldsymbol{\alpha}, \boldsymbol{\beta}, \boldsymbol{\gamma})^2
\end{aligned}
$$

由 $\boldsymbol{\alpha}, \boldsymbol{\beta}, \boldsymbol{\gamma}$ 不共面可知 $(\boldsymbol{\alpha}, \boldsymbol{\beta}, \boldsymbol{\gamma}) \neq 0$，则 $(\boldsymbol{\beta} \times \boldsymbol{\gamma}, \boldsymbol{\gamma} \times \boldsymbol{\alpha}, \boldsymbol{\alpha} \times \boldsymbol{\beta}) > 0$。

故 $\boldsymbol{\beta} \times \boldsymbol{\gamma}, \boldsymbol{\gamma} \times \boldsymbol{\alpha}, \boldsymbol{\alpha} \times \boldsymbol{\beta}$ 不共面，且定向与 $\mathbf{S}$ 一致。
...
对于平面 $\Pi (O'; \mathbf{a}, \mathbf{b})$，我们可以用其上点的矢径集合表征，记作 $\Pi (\mathbf{x}^*, \mathbf{a}, \mathbf{b}) = \{\mathbf{x} \mid \mathbf{x} = \mathbf{x}^* + \lambda \mathbf{a} + \mu \mathbf{b}, \lambda, \mu \in \mathbb{R}\}$。
...
- 事实上，$\mathbf{a}, \mathbf{b}$ 正是 $Ax + By + Cz = 0$ 的两个线性无关的解，它们恰好构成了一个**基础解系**。
...
由此可见，在右手系下，平面方程总能表示为 $Ax + By + Cz + D = 0$ 的形式。具体地，在上例中有：
...
- 此处可以类比平面解析几何中的截距式方程 $\dfrac{x}{a} + \dfrac{y}{a} = 1$。
...
由三点不共线可知 $\begin{bmatrix} x_1 & y_1 & z_1 \\ x_2 & y_2 & z_2 \\ x_3 & y_3 & z_3 \end{bmatrix}$ 满秩，于是有：
...
令 $\mathbf{n} = \dfrac{(A, B, C)}{||(A, B, C)||}$，则上式等价于 $\mathbf{n} \cdot (\mathbf{x} - \mathbf{x}^*) = 0$，即 $\mathbf{x}^*$ 在平面上且平面垂直于 $\mathbf{n}$。

我们称 $\mathbf{n}$ 为平面的单位法向量，称该形式为平面的**点法式方程**。
...
对于直线 $L(A; \mathbf{a})$，我们可以用其上点的矢径集合表征，记作 $L(\mathbf{x}^*, \mathbf{a}) = \{\mathbf{x} \mid \mathbf{x} = \mathbf{x}^* + \lambda \mathbf{a}, \lambda \in \mathbb{R}\}$。

这被称作直线的**参数方程**，其中参数为 $\lambda$。

##### 直线的点向式方程

当 $\mathbf{a}$ 的各坐标非零，我们也可以将参数方程写作 $\dfrac{x_1 - x^*_1}{a_1} = \dfrac{x_2 - x^*_2}{a_2} = \dfrac{x_3 - x^*_3}{a_3} = \lambda \in \mathbb{R}$。

省去 $\lambda$ 后，得到的就是直线的**点向式方程**。
...
- 在右手系下，**将等式解读为内积为 $0$**，有 $\mathbf{a} \perp (A_1, B_1, C_1), \mathbf{a} \perp (A_2, B_2, C_2) \Rightarrow \mathbf{a} \parallel (A_1, B_1, C_1) \times (A_2, B_2, C_2)$。因此，我们可以通过直线的一般方程给出一个 $\mathbf{a}$。
...
由于 $a_1, a_2, a_3$ 中至少一个非零（不妨设其为 $a_i$），则当 $\forall j \in \{1, 2, 3\} \backslash \{i\}$，$a_j$ 对应的行列式为零，即可推知 $a_i$ 对应的行列式为零。
...
- $\mathbf{v} = \mathbf{y} - \mathbf{x}^*$ 沿 $\mathbf{a}$ 方向的分量为 $\dfrac{(\mathbf{v}, \mathbf{a})}{||\mathbf{a}||} \cdot \dfrac{\mathbf{a}}{||\mathbf{a}||}$。
- 故其垂直于 $\mathbf{a}$ 方向的分量为 $\mathbf{v} - \dfrac{(\mathbf{v}, \mathbf{a})}{||\mathbf{a}||} \cdot \dfrac{\mathbf{a}}{||\mathbf{a}||}$。
- 因此 $d(\mathbf{y}, L) = ||\mathbf{v} - \dfrac{(\mathbf{v}, \mathbf{a})}{||\mathbf{a}||} \cdot \dfrac{\mathbf{a}}{||\mathbf{a}||}||$。
...
- (3) 当 $\operatorname{rank}(\mathbf{P}) = 4$，线性方程组 $\mathbf{P} \begin{bmatrix} x \\ y \\ z \\ w \end{bmatrix} = \mathbf{0}_4$ 仅有零解，故在 $w = 1$ 的额外要求下无解，故 $L_1, L_2$ 不交。
- 而此时 $\not\exists \alpha, \beta, \gamma, \theta$ 非全零，$\text{s.t. } \alpha \Pi_1 + \beta \Pi_2 = \gamma \Pi_3 + \theta \Pi_4$，则两者没有公共平面束，故 $L_1, L_2$ **异面**。
