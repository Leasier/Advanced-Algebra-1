...
对于平面 $\Pi (O'; \mathbf{a}, \mathbf{b})$，我们可以用其上点的矢径集合表征，记作 $\Pi = \{\mathbf{x} \mid \mathbf{x}^* + \lambda \mathbf{a} + \mu \mathbf{b}, \lambda, \mu \in \mathbb{R}\}$。
...
对于直线 $L(A; \mathbf{a})$，我们可以用其上点的矢径集合表征，记作 $L = \{\mathbf{x} \mid \mathbf{x}^* + \lambda \mathbf{a}, \lambda \in \mathbb{R}\}$。
...
因此，选定恰当的 $(A_1, B_1, C_1, D_1), (A_2, B_2, C_2, D_2)$ 后即可通过 $\begin{cases} A_1 x + B_1 y + C_1 z + D_1 = 0 \\ A_2 x + B_2 y + C_2 z + D_2 = 0 \end{cases}$ 表征解空间，其中两方程线性无关。

这一形式被称作直线的**一般方程**。

- 事实上，$\mathbf{a}$ 正是 $\begin{cases} A_1 x + B_1 y + C_1 z = 0 \\ A_2 x + B_2 y + C_2 z = 0 \end{cases}$ 的一个非零解。
- 考虑这两个用于表征直线的方程，可以发现它们各为一个**平面**的一般方程，这意味着直线总可以表示为两平面的**交线**。
...
$$
\begin{vmatrix} a_1 & a_2 & a_3 \\ x - x_0 & y - y_0 & z - z_0 \\ a_1 & a_2 & a_3 \end{vmatrix} = a_1 \begin{vmatrix} a_2 & a_3 \\ y - y_0 & z - z_0 \end{vmatrix} + a_2 \begin{vmatrix} a_3 & a_1 \\ z - z_0 & x - x_0 \end{vmatrix} + a_3 \begin{vmatrix} a_1 & a_2 \\ x - x_0 & y - y_0 \end{vmatrix} = 0
$$

由于 $a_1, a_2, a_3$ 中至少一个非零（不妨设其为 $a_i$），则当 $\forall j \in \{1, 2, 3\} \backslash \{j\}$，$a_j$ 对应的行列式非零，即可推知 $a_i$ 对应的行列式非零。
