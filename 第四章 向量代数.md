...
$$
\begin{cases}
\overrightarrow{MD} = \lambda \mathbf{b} + (1 - \lambda) \mathbf{c} \\
\overrightarrow{ME} = \mu \mathbf{c} + (1 - \mu) \mathbf{a} \\
\overrightarrow{MF} = \theta \mathbf{a} + (1 - \theta) \mathbf{b}
\end{cases}
$$

- 由于 $(A, M, D), (B, M, E), (C, M, F)$ 分别共线，有：

$$
\begin{cases}
\exists \alpha \neq 0, \text{s.t. } \alpha \mathbf{a} + \overrightarrow{MD} = \mathbf{0} \Leftrightarrow \alpha \mathbf{a} + \lambda \mathbf{b} + (1 - \lambda) \mathbf{c} = \mathbf{0} \\
\exists \beta \neq 0, \text{s.t. } \beta \mathbf{b} + \overrightarrow{ME} = \mathbf{0} \Leftrightarrow (1 - \mu) \mathbf{a} + \beta \mathbf{b} + \mu \mathbf{c} = \mathbf{0} \\
\exists \gamma \neq 0, \text{s.t. } \theta \mathbf{c} + \overrightarrow{MF} = \mathbf{0} \Leftrightarrow \theta \mathbf{a} + (1 - \theta) \mathbf{b} + \gamma \mathbf{c} = \mathbf{0}
\end{cases}
$$

- 由 $\mathbf{a}, \mathbf{b}, \mathbf{c}$ 不共线，知 $\exists ! x_0, y_0 \in \mathbb{R}_{\neq 0}, \text{s.t. } \mathbf{c} = x_0 \mathbf{a} + y_0 \mathbf{b}$，故所有非零且满足 $x \mathbf{a} + y \mathbf{b} + z \mathbf{c} = \mathbf{0}$ 的 $(x, y, z)$ 都可以表示为 $t(x_0, y_0, -1)$ 的形式，即**系数成比例**：

$$
\begin{cases}
\dfrac{\alpha}{1 - \mu} = \dfrac{\lambda}{\beta} = \dfrac{1 - \lambda}{\mu} \\
\dfrac{1 - \mu}{\theta} = \dfrac{\beta}{1 - \theta} = \dfrac{\mu}{\gamma} \\
\dfrac{\theta}{\alpha} = \dfrac{1 - \theta}{\lambda} = \dfrac{\gamma}{1 - \lambda}
\end{cases}
$$

- _Motivation：我想在保留对称性的情况下把 $\alpha, \beta, \gamma$ 消掉。_
- 考虑下面的对称等式：

$$
\dfrac{\alpha}{1 - \mu} \cdot \dfrac{\beta}{1 - \theta} \cdot \dfrac{\gamma}{1 - \lambda} = \dfrac{\lambda}{\beta} \cdot \dfrac{\mu}{\gamma} \cdot \dfrac{\theta}{\alpha} = \dfrac{1 - \lambda}{\mu} \cdot \dfrac{1 - \mu}{\theta} \cdot \dfrac{1 - \theta}{\lambda}
$$

- 将等式中的三项依次记作 $\mathcal{A}, \mathcal{B}, \mathcal{C}$，则 $\mathcal{A} \mathcal{B} = \dfrac{1}{\mathcal{C}}$，解得 $\mathcal{A} = \mathcal{B} = \mathcal{C} = 1$。
- 故 $\dfrac{|CD|}{|BD|} \cdot \dfrac{|AE|}{|CE|} \cdot \dfrac{|BF|}{|AF|} = \dfrac{1}{\mathcal{C}} = 1$。
